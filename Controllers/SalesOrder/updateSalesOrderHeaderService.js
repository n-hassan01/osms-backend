...
router.put("/:header_id", async (req, res, next) => {
  ...
  const { shippingMethodCode, description, lastUpdatedBy } = req.body;

  const date = new Date();

  await pool.query(
    "UPDATE oe_order_headers_all SET shipping_method_code=$1, description=$2, last_updated_by=$3 WHERE header_id=$4;",
    [shippingMethodCode, description, lastUpdatedBy, headerId],
    (error, result) => {
      try {
